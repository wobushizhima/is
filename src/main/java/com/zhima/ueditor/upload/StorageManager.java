package com.zhima.ueditor.upload;
...
//			UploadResult result = baseFileService.upload(dataBuf, picName, "OM", null);
			Date date=new Date();
			picName= date.getTime()+"_"+picName;
			File dest = new File(fileurl + picName);
			// 检测是否存在目录
			if (!dest.getParentFile().exists()) {
				dest.getParentFile().mkdirs();
			}
			OutputStream out = new FileOutputStream(dest);
			InputStream myIs = new ByteArrayInputStream(dataBuf);
			byte[] buff = new byte[1024];
			int len = 0;
			while((len=myIs.read(buff))!=-1){
